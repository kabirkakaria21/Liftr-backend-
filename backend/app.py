...
app = Flask(__name__)
app.config['mongodb+srv://kabirkakaria:<lonerwithboner>@cluster0.jfnqp.mongodb.net/myFirstDatabase?retryWrites=true&w=majority'] = 'secret!'
...
def connect_handler():
    logger.debug(
        f"connection from sid: {request.sid} ip: {request.remote_addr}")
...
def handle_start_session():
    logger.debug(
        f"start_session - client: {g.client} session: {g.client.session}")
    c: Client = g.client
    s: Session = c.session
...
def handle_select_exercise(data):
    logger.debug(
        f"select_exercise - client: {g.client} session: {g.client.session} data: {data}")
    if isinstance(g.client, MobileApp):
...
def handle_start_exercise(data):
    logger.debug(
        f"start_exercise - client: {g.client} session: {g.client.session} data: {data}")
    if isinstance(g.client, MobileApp):
...
def handle_make_critique(data):
    logger.debug(
        f"make_critique - client: {g.client} session: {g.client.session} data: {data}")
...
def handle_start_exercise(data):
    logger.debug(
        f"update_reps - client: {g.client} session: {g.client.session} data: {data}")
